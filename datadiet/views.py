...
def db_create_post(db, title, body, hyperlink, username, diettag):
    ...
    date = cur.fetchone()[0]
    cur.execute("""INSERT INTO postAggregates (post_id, rating, hotness, total_likes, total_dislikes, total_comments) VALUES (%s, %s, LOG10(TO_SECONDS(%s)), %s, %s, %s)""", (last_row_id, 0, date, 0, 0, 0))
    db.commit()
    cur=db.cursor()
